                            ...
                            <tbody>
                                ...
                                @foreach ($MainQuestion as $index => $item)
                                    <tr>
                                        ...
                                        <td class="text-start">{{ $item->main_question_name }}</td>
                                        @php
                                            $multiplyFactor = $item->main_question_multiply ?: 1; // ใช้ค่า default เป็น 1 หากไม่มีการตั้งค่า
                                        @endphp
                                        @for ($score = 5; $score >= 1; $score--)
                                            <td>
                                                <div class="form-check d-flex justify-content-center">
                                                    <input class="form-check-input border-dark" type="radio"
                                                        name="main_score[{{ $index }}]"
                                ...
                                <tr>
                                    <td >
                                        สมรรถนะอื่นๆตามที่ส่วนราชการกำหนด
                                    </td>
                                </tr>
                                @foreach ($OtherQuestion as $index => $item)
                                    <tr>
                                        ...
                                        @php
                                            $multiplyFactor = $item->other_question_multiply ?: 1; // ใช้ค่า default เป็น 1 หากไม่มีการตั้งค่า
                                        @endphp
                                        @for ($score = 5; $score >= 1; $score--)
                                            <td>
                                                <div class="form-check d-flex justify-content-center">
